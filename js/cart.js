...
(function () {
    ...
    function wireAddButtons() {
        ...
        addBtns.forEach(btn => {
            btn.addEventListener('click', e => {
                ...
                const priceNode = card.querySelector('.card__price');
                let price = 0;
                if (priceNode) {
    ...
    function wireDetailAddButton() {
        const btn = document.querySelector('.addToCart');
        if (!btn) return;

        btn.addEventListener('click', () => {
            const detail = document.querySelector('.detailBook__content');
            if (!detail) return;

            const titleNode = detail.querySelector('.detailBook__title');
            const authorNode = detail.querySelector('.detailBook__author');
            const priceNode = detail.querySelector('.detailBook__price');
